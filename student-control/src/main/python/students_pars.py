...
def pars(text):
    ...
    ser_list = json.dumps(_list, ensure_ascii=False).replace('"{', '{').replace('}"', '}').replace('\\', '').encode('utf-8')
    #decoded = ser_list.decode() 
    #r = requests.post('http://localhost:8080/api/data/students', ser_list)
    print(str(ser_list, 'utf-8'))
